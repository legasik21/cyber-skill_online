...
export interface ChatOrder {
  ...
  /** True when no module price is on record — route it for a manager to confirm pricing. */
...
export async function sendChatOrderNotification(order: ChatOrder): Promise<boolean> {
  ...
  const tbd = order.needsPriceConfirmation === true;

  let text = tbd
    ? `🛒 *NEW IN-CHAT ORDER — PRICE TBD (manager to confirm)*\n\n`
    : `🛒 *NEW IN-CHAT ORDER*\n\n`;
  text += `🎮 Service: *${escapeMarkdown(order.service)}*\n`;
  text += `📋 Details: ${escapeMarkdown(order.summary)}\n`;
  text += tbd
    ? `💵 Price: *manager to confirm* (auto-quote not on record)\n`
    : `💵 Quoted price: ${escapeMarkdown(order.price)}\n`;
...
/** Contact handle collected from the visitor, if any, to attach to an escalation. */
...
/**
 * Escalation notifier — a chat needs a human. Posts to the notifications group
 * (TELEGRAM_CHAT_ID) with the conversation id, the reason, and any contact handle
 * already collected. This is the safety net behind every "connect you with a team
 * member": an escalation that pings nobody is the bug this replaces. Returns whether
 * a notification was actually delivered (so callers can log a hard failure).
 */
export async function sendEscalationNotification(
  ...
  if (!TELEGRAM_CHAT_ID) {
    console.error(
      'Telegram notifications chat (TELEGRAM_CHAT_ID) not configured — CANNOT alert on escalation',
    );
    return false;
  }
  ...
  let text = `🚨 *LEAD — human needed*\n\n`;
  text += `🧭 Reason: ${escapeMarkdown(reason)}\n`;
  if (contact?.handle) {
    text += `📨 Contact: *${escapeMarkdown(contact.platform || 'unknown')}* — \`${escapeMarkdown(contact.handle)}\`\n`;
  } else {
    text += `📨 Contact: _none captured yet — reply in the chat_\n`;
  }
  text += `🆔 Conversation: \`${convShort}\`\n`;
  text += `🔗 [Open Admin Panel](${adminUrl})\n`;
  text += `\n⏰ ${timestamp}`;
